...
class OdriveNode(Node):
    def __init__(self):
        ...
        self.timer_recalc_odom = self.create_timer(
            1 / self.odom_calc_hz, self.fast_timer
        )
        ...
        self.raw_odom_publisher_encoder_right = self.create_publisher(
            Float64, "odrive/right/raw_odom/encoder", self.odom_calc_hz
        )
        self.raw_odom_publisher_vel_left = self.create_publisher(
            Float64, "odrive/left/raw_odom/velocity", self.odom_calc_hz
        )
        self.raw_odom_publisher_vel_right = self.create_publisher(
            Float64, "odrive/right/raw_odom/velocity", self.odom_calc_hz
        )
        self.odom_publisher = self.create_publisher(
            Odometry, self.odom_topic, self.odom_calc_hz
        )
        self.current_publisher_left = self.create_publisher(
            Float64, "odrive/left/current", self.odom_calc_hz
        )
        self.current_publisher_right = self.create_publisher(
            Float64, "odrive/right/current", self.odom_calc_hz
        )
        self.i2t_publisher_left = self.create_publisher(
            Float64, "odrive/left/i2t", self.odom_calc_hz
        )
    ...
    def init_params(self):
        self.declare_parameter(
            "simulation_mode", False, ParameterDescriptor(description="Simulation mode")
        )
        ...
        self.declare_parameter(
            ...
            True,
            ParameterDescriptor(
                description="NOT YET IMPLEMENTED ! Weather to publish joint angles"
            ),
        )
        self.declare_parameter(
        ...
        self.declare_parameter(
            ...
            0,
            ParameterDescriptor(
                description="NOT YET IMPLEMENTED! if right calibrates first, this should be 0, else 1"
            ),
        )  #
        self.declare_parameter(
            "wheel_track",
        ...
        self.declare_parameter(
            ...
            0.537,
            ParameterDescriptor(
                description="Used to translate velocity commands in m/s into motor rpm"
            ),
        )
        ...
        self.declare_parameter(
            ...
            True,
            ParameterDescriptor(
                description="If True, the node publishes odometry information to the specified odom_topic topic"
            ),
        )
        self.declare_parameter(
            "publish_odom_tf",
        ...
        self.declare_parameter(
            ...
            "odrive/odom",
            ParameterDescriptor(
                description="The topic to publish estimated odometry information."
            ),
        )
        self.declare_parameter(
            "odom_frame",
        ...
        self.declare_parameter(
            ...
            50,
            ParameterDescriptor(
                description="The frequency of the odom calculations and other high rate calculations"
            ),
        )
        ...
        self.publish_joint_angles = (
            self.get_parameter("publish_joint_angles").get_parameter_value().bool_value
        )
        self.publish_temperatures = (
            self.get_parameter("publish_temperatures").get_parameter_value().bool_value
        )

        self.axis_for_right = (
            self.get_parameter("axis_for_right").get_parameter_value().double_value
        )
        self.wheel_track = (
            self.get_parameter("wheel_track").get_parameter_value().double_value
        )
        self.tyre_circumference = (
            self.get_parameter("tyre_circumference").get_parameter_value().double_value
        )

        self.connect_on_startup = (
            self.get_parameter("connect_on_startup").get_parameter_value().bool_value
        )
        self.calibrate_on_startup = (
            self.get_parameter("calibrate_on_startup").get_parameter_value().bool_value
        )
        self.engage_on_startup = (
            self.get_parameter("engage_on_startup").get_parameter_value().bool_value
        )

        self.has_preroll = (
            self.get_parameter("use_preroll").get_parameter_value().bool_value
        )

        self.publish_curr = (
            self.get_parameter("publish_current").get_parameter_value().bool_value
        )
        self.publish_raw_odom = (
            self.get_parameter("publish_raw_odom").get_parameter_value().bool_value
        )
        ...
        self.publish_tf = (
            self.get_parameter("publish_odom_tf").get_parameter_value().bool_value
        )
        self.odom_topic = (
        ...
        self.odom_calc_hz = (
            self.get_parameter("odom_calc_hz").get_parameter_value().integer_value
        )
    ...
    def cmd_vel_callback(self, msg):
        left_linear_val, right_linear_val = self.convert_linear_angular_to_l_r_linear(
    ...
    def disengage_on_stop(self):
        if (abs(self.last_left_linear_val) < 0.01) and (
            abs(self.last_right_linear_val) < 0.01
        ):
    ...
    def update_odometry(self, time_now):
        ...
        )  # -self.driver.left_vel_estimate()   # units: encoder counts/s
    ...
    def publish_current(self, time_now):
        ...
        if (
            self.left_energy_acc > self.i2t_error_threshold
            or self.right_energy_acc > self.i2t_error_threshold
        ):
            ...
            if (
                self.left_energy_acc < self.i2t_resume_threshold
                and self.right_energy_acc < self.i2t_resume_threshold
            ):
    ...
    def publish_odometry(self, time_now):
        ...
        if delta_pos_l > half_cpr:
            delta_pos_l = delta_pos_l - 1.0 * self.encoder_cpr
        elif delta_pos_l < -half_cpr:
            delta_pos_l = delta_pos_l + 1.0 * self.encoder_cpr
        if delta_pos_r > half_cpr:
            delta_pos_r = delta_pos_r - 1.0 * self.encoder_cpr
